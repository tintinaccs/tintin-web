...
function groupedNotifications(items) {
  ...
  return [...groups.values()].map(group => {
    ...
    const actor = String(newest.actorName || 'Una clienta').trim();
    const kind = String(newest.kind || '').replace(/^store_/, '');
    ...
    return {
      ...
      title: kind.includes('like') ? `${actor} y ${Math.max(1, count - 1)} persona${count === 2 ? '' : 's'} más dieron Me gusta` : newest.title,
      body: kind.includes('like') ? `${target}${newest.productName && newest.targetType !== 'product' ? ` · ${newest.productName}` : ''} · ${count} Me gusta en total` : newest.body,

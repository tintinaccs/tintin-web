...
import { initConnectionsFlow } from "./flujo-conexiones/flujo-conexiones-admin.js?v=tintin-20260922-admin-guard-live-1";
...
async function startAdminAuthGuard() {
  subscribeSession(async snapshot => {
    ...
    if (snapshot.status === AUTH_STATES.UNKNOWN) {
      // UNKNOWN no es una cuenta y tampoco autoriza una redirección. Mantener
      // el documento en Admin evita el circuito Admin -> login -> Admin cuando
      // Firebase/IndexedDB todavía no pudo confirmar la restauración.
    ...
    if (!user) {
      ...
      // Una ausencia no confirmada tampoco debe navegar a login: el botón del
      // overlay permite hacerlo de forma explícita, sin iniciar otro rebote.

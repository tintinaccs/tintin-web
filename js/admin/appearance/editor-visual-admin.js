...
function normalizedConfig(raw) {
  ...
  base.customBlocks = (Array.isArray(raw?.customBlocks) ? raw.customBlocks : []).map(block => ({ ...block, afterSection:safeBlockAnchor(block?.afterSection), style:normalizeStyle(block?.style), items:Array.isArray(block?.items) ? block.items : [], images:Array.isArray(block?.images) ? block.images : [] })).filter(block=>Boolean(block.afterSection));
  return base;
}

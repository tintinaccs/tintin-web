...
function css(){
 if(!document.getElementById('tt-ui-quality-css')){var l=document.createElement('link');l.id='tt-ui-quality-css';l.rel='stylesheet';l.href=new URL('../css/ui-quality.css',import.meta.url).href;document.head.appendChild(l)}
 if(!document.getElementById('tt-unified-theme-css')){var t=document.createElement('link');t.id='tt-unified-theme-css';t.rel='stylesheet';t.href=new URL('../css/tintin-unified-theme.css',import.meta.url).href;document.head.appendChild(t)}
}
...
function singleLogoLoader(){var logo='assets-tintin/images/general/logo.png';document.querySelectorAll('#tt-loader-line,#tt-loader-text,#tt-loader-dots,.tt-splash-line,#tt-intro-fallback').forEach(function(el){el.remove()});document.querySelectorAll('#tt-loader-logo,#tt-intro-logo').forEach(function(img){if(!img||img.dataset.ttSingleLogo)return;img.dataset.ttSingleLogo='1';img.alt='';img.removeAttribute('aria-label');if(img.src.indexOf('logo.png')===-1)img.src=logo;img.addEventListener('error',function(){img.style.display='none'},{once:true})})}

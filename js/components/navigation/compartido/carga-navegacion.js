...
const CART_RUNTIME_URL = '../../../components/cart/sincronizacion-carrito.js?v=tintin-20260923-auth-cache-coherence-1';
...
function loadAuthRuntime() {
  ...
  if (!authRuntimePromise) {
    authRuntimePromise = import('../../../core/auth/navegacion-autenticacion.js?v=tintin-20260923-auth-cache-coherence-1').catch(error => {
...
function loadNavigationBehaviors() {
  ...
  navigationBehaviorsPromise = Promise.resolve(controllerReady)
    ...
    .then(() => Promise.allSettled([
      ...
      import('./control-busqueda.js?v=tintin-20260923-auth-cache-coherence-1'),
    ]))
    .then(results => {
      reportRuntimeFailures(results);

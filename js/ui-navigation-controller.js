  ...
  class SurfaceController {
    ...
    onResize() {
      ...
      this.resizeFrame = requestAnimationFrame(() => {
        ...
        if (this.surface !== 'none') this.close('breakpoint', { restoreFocus: false });
      });
    }
    ...
    async animateMorph(trigger, target, opening, token) {
      if (reduceMotion.matches || !trigger?.isConnected || !target?.isConnected || !this.morphLayer) return;
      ...
      const animation = layer.animate(
        ...
        { duration: opening ? 360 : 260, easing: opening ? 'cubic-bezier(.2,.82,.2,1)' : 'cubic-bezier(.4,0,.6,1)', fill: 'both' }
      );
    ...
    lockScroll() {
      if (document.documentElement.classList.contains('tt-surface-locked')) return;
      this.lockedScrollY = scrollY || document.documentElement.scrollTop || 0;
      this.savedBodyStyle = document.body.getAttribute('style');
      document.documentElement.classList.add('tt-surface-locked');
      document.documentElement.style.overflow = 'hidden';
      Object.assign(document.body.style, {
        position: 'fixed', top: `-${this.lockedScrollY}px`, left: '0', right: '0',
    ...
    unlockScroll() {
      if (!document.documentElement.classList.contains('tt-surface-locked')) return;
      document.documentElement.classList.remove('tt-surface-locked');
      document.documentElement.style.overflow = '';
      if (this.savedBodyStyle == null) document.body.removeAttribute('style');
      else document.body.setAttribute('style', this.savedBodyStyle);
    ...
    setBackgroundInert(activeElement, inert) {
      if (inert) {
        ...
        this.inertState.forEach((value, node) => { if (node.isConnected) node.inert = value; });
        this.inertState.clear();
      }
    }
    ...
    emit() {
      dispatchEvent(new CustomEvent('tintin:surface-change', { detail: { state: this.state, surface: this.surface } }));
    }
    ...
    async open(name, trigger = document.activeElement) {
      ...
      if (this.surface !== 'none') await this.close('switch', { restoreFocus: false, preserveOperation: true });
    ...
    async close(reason = 'api', { restoreFocus = true, preserveOperation = false } = {}) {
      ...
      await this.animateMorph(returnTarget, config.element, false, token);
        ...
        : trigger.id === 'btn-menu-tablet' ? 'tablet-menu'
          : trigger.id === 'tabbar-tienda' ? 'mobile-shop'
            : trigger.matches('[data-nav-action="search"],#tabbar-search') ? 'search'
              : trigger.matches('[data-nav-action="cart"],#tabbar-cart') ? 'cart' : 'account';
    ...
    onKeyDown(event) {
      ...
      if (!nodes.length) { event.preventDefault(); config.element.focus(); return; }

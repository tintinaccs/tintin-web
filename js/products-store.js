...
const ALL_CACHE_KEY = 'products:all';
...
async function fetchAllProducts() {
  ...
  const products = snapshot.docs.map(item => mapProduct(item.id, item.data()));
  const normalized = normalizeList(products);
  writeCached(ALL_CACHE_KEY, normalized);
  return publish(normalized, 'server');
}
...
export async function loadProductPage(options = {}) {
  ...
  const fullCache = readCached(ALL_CACHE_KEY, ALL_CACHE_TTL);
  if (Array.isArray(fullCache) && fullCache.some(item => String(item.id) === id)) {
    return publish(fullCache, 'cache');
  }

...
var MIN_SHOW_MS=820,SAFETY_MS=5200,START=Date.now();
...
var DEFAULT_LOGO_SRC=resolveAsset('assets-tintin/images/general/logo.png');
var LOGO_SRC=savedLogo()||DEFAULT_LOGO_SRC;
try{document.documentElement.classList.add('tt-loader-active');document.documentElement.style.setProperty('--tt-loader-logo-url',cssUrl(LOGO_SRC));}catch(e){}
var CSS=[
'html.tt-loader-active:before{content:"";position:fixed;inset:0;z-index:2147482999;background:#FFF6FA var(--tt-loader-logo-url) center center/220px auto no-repeat!important;pointer-events:none;opacity:1;visibility:visible;transition:opacity .38s ease,visibility .38s ease}',
'html.tt-loader-active:after{content:"";position:fixed;left:50%;top:50%;width:256px;height:256px;z-index:2147482999;border-radius:50%;border:3px solid rgba(212,106,138,.15);border-top-color:#D46A8A;transform:translate(-50%,-50%) rotate(0deg);animation:tt-ring-spin 1s linear infinite;pointer-events:none}',
'html.tt-loader-active.tt-loader-done:before,html.tt-loader-active.tt-loader-done:after{opacity:0;visibility:hidden}',
...
'body.tt-scroll-locked{position:fixed!important;left:0!important;right:0!important;width:100%!important;overflow:hidden!important;overscroll-behavior:none;touch-action:none!important}',
...
'#tt-loader-spin-wrap{position:relative;display:flex;align-items:center;justify-content:center;min-width:220px;min-height:220px}',
'#tt-loader-logo{position:relative;z-index:1;width:clamp(126px,18vw,210px);max-width:72vw;height:auto;object-fit:contain;display:block;opacity:0;transform:translateY(8px) scale(.96);filter:drop-shadow(0 8px 22px rgba(212,106,138,.18));user-select:none;pointer-events:none}',
'#tt-loader-spin-wrap.tt-ready #tt-loader-logo{animation:tt-logo-only-in .48s cubic-bezier(.22,.61,.36,1) both}',
'#tt-loader-fallback{position:absolute;z-index:0;font-family:Georgia,serif;font-size:clamp(32px,7vw,58px);font-weight:700;letter-spacing:.16em;color:#B84C72;opacity:.18;user-select:none;pointer-events:none}',
'#tt-loader-ring{position:absolute;top:50%;left:50%;width:calc(100% + 34px);height:calc(100% + 34px);border-radius:50%;border:3px solid rgba(212,106,138,.15);border-top-color:#D46A8A;transform:translate(-50%,-50%) rotate(0deg);opacity:0;pointer-events:none}',
'#tt-loader-spin-wrap.tt-ready #tt-loader-ring{animation:tt-ring-fade-in .3s ease .2s forwards,tt-ring-spin 1s linear infinite .2s}',
'@keyframes tt-logo-only-in{from{opacity:0;transform:translateY(8px) scale(.96)}to{opacity:1;transform:translateY(0) scale(1)}}',
'@keyframes tt-ring-fade-in{to{opacity:1}}',
'@keyframes tt-ring-spin{from{transform:translate(-50%,-50%) rotate(0deg)}to{transform:translate(-50%,-50%) rotate(360deg)}}',
'@media (max-width:600px){html.tt-loader-active:before{background-size:154px auto!important}html.tt-loader-active:after{width:190px;height:190px}#tt-loader-logo{width:clamp(112px,34vw,160px)}#tt-loader-spin-wrap{min-width:168px;min-height:168px}}',
'@media (min-width:601px) and (max-width:1120px){html.tt-loader-active:before{background-size:180px auto!important}html.tt-loader-active:after{width:218px;height:218px}#tt-loader-logo{width:clamp(122px,22vw,180px)}}',
'@media (prefers-reduced-motion:reduce){#tt-loader,html.tt-loader-active:before,html.tt-loader-active:after{transition:opacity .01s linear}html.tt-loader-active:after,#tt-loader-ring{display:none}#tt-loader-spin-wrap.tt-ready #tt-loader-logo{animation:none;opacity:1;transform:none}}'
].join('');
...
var el=document.createElement('div');el.id='tt-loader';el.setAttribute('aria-hidden','true');el.setAttribute('role','presentation');el.dataset.state='show';el.innerHTML='<div id="tt-loader-spin-wrap"><span id="tt-loader-fallback">TINTIN</span><span id="tt-loader-ring"></span><img id="tt-loader-logo" src="'+LOGO_SRC+'" alt="" draggable="false" fetchpriority="high" width="220" height="220"></div>';
...
waitForBody();
function hideNow(){if(hidden)return;hidden=true;el.dataset.state='out';el.classList.add('tt-out');document.documentElement.classList.add('tt-loader-done');setTimeout(function(){try{document.documentElement.classList.remove('tt-loader-active','tt-loader-done')}catch(e){}},460)}
...
function show(){hidden=false;contentReady=false;logoReady=!!(logo&&logo.complete);el.dataset.state='show';el.classList.remove('tt-out');document.documentElement.classList.add('tt-loader-active');document.documentElement.classList.remove('tt-loader-done')}
function setText(){}

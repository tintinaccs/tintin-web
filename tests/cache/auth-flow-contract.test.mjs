...
test('admin conserva UNKNOWN y ofrece recuperación ante una ausencia de sesión', () => {
  ...
  assert.match(admin, /UNKNOWN no es una cuenta y tampoco autoriza una redirección/);
  assert.doesNotMatch(admin, /clearAdminAuthHandoffWithDiagnostic\(\);\s*window\.location\.replace\('login\.html'\)/);
});

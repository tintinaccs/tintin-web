...
test('checkout espera la restauración de Auth antes de tratar al usuario como invitado', () => {
  assert.match(checkoutPage, /subscribeSession\(snapshot =>/);
  assert.doesNotMatch(checkoutPage, /checkoutAuthStateReady\.then/);
  assert.match(checkoutPage, /sessionReady/);
  assert.match(hardening, /AUTH_STATES\.UNKNOWN/);
  assert.match(hardening, /subscribeSession\(snapshot =>/);

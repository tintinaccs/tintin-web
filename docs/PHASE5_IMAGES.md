...
- Desde entonces se agregó un widget de carga (`js/components/images/image-upload-widget.js`) y una biblioteca multimedia (`js/admin/products/admin-media-library-ui.js`, `js/components/images/media-library.js`) que suben el archivo a Cloudinary (firmado vía `functions/api/cloudinary-sign-upload.js`, borrado vía `functions/api/cloudinary-delete.js`) y guardan la URL resultante en los mismos campos de esta tabla — sigue sin ser Firebase Storage, pero ya no es solo "pegar una URL a mano".

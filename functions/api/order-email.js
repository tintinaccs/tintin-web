...
async function verifyFirebaseUser(idToken) {
  ...
  if (!response.ok) {
    ...
    if (
      /INVALID_ID_TOKEN|TOKEN_EXPIRED|USER_NOT_FOUND/i.test(reason)
    ) {
      throw new Error(
...
async function fetchOrder(
  ...
  const endpoint =
    `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT_ID}/databases/(default)/documents/orders/${encodeURIComponent(safeOrderId)}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `No se pudo leer el pedido (HTTP ${response.status}${reason ? `: ${reason}` : ''}).`
    );
  }
...
function shippingLabel(order) {
  ...
  const base = {
    delivery:
      'Delivery (Zona Central)',

    encomienda:
      'Encomienda (Interior)',

    retiro:
      'Retiro en San Lorenzo'
  }[method] ||
    method ||
    'A coordinar';
  ...
  if (
    ...
    if (mode === 'agencia') {
      return (
        `${base} — retiro en agencia`
      );
    }

    if (mode === 'puerta') {
      return (
        `${base} — entrega en puerta`
      );
    }
  }
...
function customerEmail(
  ...
  const rows =
    ...
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #f2e4e9;color:#2b2b2b">
        ...
        ${
          item.variant
            ? `<div style="font-size:12px;color:#7b6f72">${escapeHtml(item.variant)}</div>`
            : ''
        }
      </td>
  ...
  const textItems =
    items
      .map(
        item =>
          `${item.qty}x ${clean(item.name, 180)} — ${fmtPrice(
            Number(item.price || 0) *
            Number(item.qty || 0)
          )}`
      )
  ...
  <div style="max-width:560px;margin:0 auto;padding:32px 16px">
    <div style="background:#ffffff;border:1px solid #f1e4e7;border-radius:20px;overflow:hidden">
      ...
      <div style="background:linear-gradient(135deg,#c6557d,#8e274d);padding:26px 24px;text-align:center">
        ...
        <div style="margin-top:6px;font-size:12.5px;font-weight:600;letter-spacing:.06em;color:rgba(255,255,255,.78)">
          PEDIDO #${escapeHtml(shortId)}
        </div>
      </div>
      ...
      <div style="padding:30px 28px">
        ...
        <p style="margin:0 0 16px;font-size:14.5px">
          Hola <strong>${escapeHtml(order.userName || 'Tintina')}</strong>,
        </p>
        ...
        <table style="width:100%;border-collapse:collapse;background:#fdf6f9;border-radius:14px">
          ...
          <tr>
            <td style="padding:12px 14px;font-size:13px;color:#7b6f72">
              Subtotal
            </td>
          ...
          <tr>
            <td style="padding:0 14px 12px;font-size:13px;color:#7b6f72">
              Envío
            </td>

            <td style="padding:0 14px 12px;font-size:13px;text-align:right">
              ${
          ...
          <tr>
            <td style="padding:12px 14px;font-size:14.5px;font-weight:700;color:#ad3f67;border-top:1px solid #f1e4e7">
              Total
            </td>

            <td style="padding:12px 14px;font-size:14.5px;text-align:right;font-weight:700;color:#ad3f67;border-top:1px solid #f1e4e7">
              ${escapeHtml(
                fmtPrice(
                  order.total
                )
    ...
    <p style="margin:18px 0 0;text-align:center;font-size:11px;color:#b6a7ac">
      Tintin Accesorios &amp; Relojes
    </p>
  </div>
</body>
</html>`;
...
Podés responder directamente a este correo para comunicarte con Tintin.`;
  ...
  return {
    subject:
      `Recibimos tu pedido #${shortId} — Tintin`,
    html,
    text
  };
}
...
function adminEmail(
  ...
  const itemRows =
    ...
    <tr>
      ...
      <td style="padding:9px 0;border-bottom:1px solid #f2e4e9;text-align:right">
        ${escapeHtml(
          fmtPrice(
            Number(
              item.price || 0
...
<body style="margin:0;background:#fdf1f5;font-family:Montserrat,Helvetica,Arial,sans-serif;color:#2b2226">
  ...
  /*
   * También usamos cityLabel en la versión
   * de texto plano del correo.
   */
  const text =
...
Super Admin: ${ADMIN_PANEL}`;
  ...
  return {
    subject:
      `Nuevo pedido #${shortId} — ${STORE_NAME}`,
    html,
    text
  };
}
...
async function sendOrderEmails({
  ...
  if (sendCustomer) {
    ...
    if (
      !emailIsValid(recipient)
    ) {
      customerSent = false;

      errors.push(
        'Clienta: correo inválido'
      );
...
export async function onRequest(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        'Este pedido no pertenece a la cuenta iniciada.'
      );
    }
    ...
    if (
      ...
      throw new Error(
        'El correo del pedido no coincide con la cuenta iniciada.'
      );
    }
    ...
    if (
      ...
      throw new Error(
        'Solo el Super Admin puede reenviar correos.'
      );
    }

    const sendAdmin =
      body.sendAdmin !== false;

    const sendCustomer =
      body.sendCustomer !== false;

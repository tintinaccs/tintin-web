...
function isGithubRateLimited(error){const message=String(error?.message||'');return error?.status===429||(error?.status===403&&/rate limit|api rate limit|secondary rate limit/i.test(message));}
function safeError(error,fallback){const message=String(error?.message||'');if(/sesión venció/i.test(message))return'La sesión venció; volvé a iniciar sesión.';if(/solo el super admin|correo verificado/i.test(message))return'La sesión no corresponde al Super Admin autorizado.';if(isGithubRateLimited(error))return'GitHub alcanzó temporalmente el límite de consultas. Volvé a actualizar en unos minutos.';if(error?.status===403)return'GitHub rechazó la operación por permisos insuficientes.';if(error?.status===404)return'No se encontró el workflow del Diagnóstico Maestro.';return fallback;}
...
export async function onRequest(context){
  ...
  try{await requireSuperAdmin(request);if(request.method==='GET')return jsonResponse(await handleGet(env),200,origin,requestUrl);const result=await handlePost(request,env);return jsonResponse(result.body,result.status,origin,requestUrl);}catch(error){console.error('[master-diagnostics]',error?.message||error);const fallback=request.method==='POST'?'No se pudo iniciar el Diagnóstico Maestro.':'No se pudo consultar el Diagnóstico Maestro.';const message=safeError(error,fallback);const authFailure=error?.status===401 || /sesión|super admin|correo verificado|autenticación/i.test(String(error?.message||''));const rateLimited=isGithubRateLimited(error);const status=authFailure?401:(rateLimited?503:(error?.status===403?502:500));const responseHeaders=rateLimited?{'retry-after':'60'}:{};return new Response(JSON.stringify({ok:false,error:message,code:rateLimited?'github_rate_limited':(error?.status===403?'github_forbidden':'diagnostics_unavailable'),retryAfterSeconds:rateLimited?60:undefined}),{status,headers:{...corsHeaders(origin,requestUrl),...responseHeaders}});}
}

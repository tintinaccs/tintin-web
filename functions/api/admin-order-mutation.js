...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!raw || new TextEncoder().encode(raw).byteLength > 32 * 1024) throw new Error('Solicitud inválida.');
    const body = JSON.parse(raw);
    const result = await applyOrderAdminMutation(env, body, {
      uid: actor.uid,
      email: actor.email,
      role: 'superadmin',
      origin: 'superadmin',

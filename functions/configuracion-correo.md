...
> `js/email/configuracion-correo.js` con la URL nueva.
...
en el JS público del sitio (`js/email/configuracion-correo.js`), así que cualquiera que
...
**Ya hecho — estado actual (migración completada):** `js/email/configuracion-correo.js` ya apunta a la
...
rota la implementación, hay que actualizar solamente la URL en `js/email/configuracion-correo.js`.
...
- Revisá que la URL en `js/email/configuracion-correo.js` termine en `/exec` (no `/dev`)
...
- Si el correo sigue llegando "De: tintinaccs@gmail.com", es porque `js/email/configuracion-correo.js`

 ...
 */
export async function injectCodeStudioRuntime(
  response,
  requestMethod = 'GET',
  runtimeUrl = '',
  stylesheetUrl = '',
  ...
  const restoreScript = String(restoreRuntimeUrl || '').trim();
  ...
  if (!/^\/js\/admin\/estudio-codigo\/restaurar-estudio-codigo-admin\.js\?v=[A-Za-z0-9._-]+$/.test(restoreScript)) {
    console.error('[code-studio] URL de runtime de restauración inválida.');
    return response;
  }
  ...
  const restoreTag = `<script type="module" src="${restoreScript}"></script>`;
  let output = html;

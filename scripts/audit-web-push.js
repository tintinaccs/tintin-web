...
const pushCore = read('cloudflare/push-core.js');
const pushService = read('cloudflare/push-service.js');
const clientModule = read('js/admin/notifications/push-notifications.js');
const deepLink = read('js/admin/notifications/push-order-deeplink.js');
const appsScript = read('apps-script/Phase4CreateOrder.gs');
...
check('El token OAuth se reutiliza mientras siga vigente', read('cloudflare/firebase-admin-lite.js').includes('accessTokenCache'));
...
check('La cuenta de servicio sale de una variable de Cloudflare', read('cloudflare/firebase-admin-lite.js').includes('FIREBASE_SERVICE_ACCOUNT_JSON'));
...
check('Existe el módulo del cliente', exists('js/admin/notifications/push-notifications.js'));
check('admin.html carga el módulo', adminHtml.includes('js/admin/notifications/push-notifications.js'));
check('admin.html carga el enlace directo al pedido', adminHtml.includes('js/admin/notifications/push-order-deeplink.js'));
...
const publicSurfaces = ['admin.html', 'firebase-messaging-sw.js', 'admin-manifest.json',
  'js/admin/notifications/push-notifications.js', 'js/admin/notifications/push-order-deeplink.js']
  .map(read).join('\n');
...
console.log('\n== Documentación ==');
check('Existe docs/FIREBASE_WEB_PUSH_SETUP.md', exists('docs/FIREBASE_WEB_PUSH_SETUP.md'));
if (exists('docs/FIREBASE_WEB_PUSH_SETUP.md')) {
  const doc = read('docs/FIREBASE_WEB_PUSH_SETUP.md');

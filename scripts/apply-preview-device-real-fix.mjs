...
const OLD_VERSION = 'tintin-20260812-preview-arriba-1';
const NEW_VERSION = 'tintin-20260812-preview-dispositivos-reales-1';

// 1) El iframe debe conservar el ancho real del dispositivo. Si el panel admin
// es más angosto, el scroll pertenece al stage; el viewport embebido NO se encoge.
{
  ...
  text = replaceExact(
    ...
    '.visual-studio-v2 .visual-preview-stage{flex:1;min-height:0;padding:24px;overflow:auto;align-items:flex-start;justify-content:flex-start;background:radial-gradient(circle at 50% 20%,#f7f7f8,#e5e6e8 70%)}',
    'stage preview'
  );
  text = replaceExact(
    text,
    ...
    '.visual-studio-v2 .visual-preview-stage iframe{flex:0 0 auto;margin-inline:auto;height:100%;min-height:680px;max-height:none;border:1px solid #d8d9dc;border-radius:12px;box-shadow:0 16px 50px rgba(20,20,30,.12);background:#fff;transition:max-width .22s ease,width .22s ease}',
    'iframe base'
  );
  text = replaceExact(
    text,
    ...
    '.visual-studio-v2 .visual-preview-stage[data-device=mobile] iframe{width:390px;min-width:390px;max-width:390px}.visual-studio-v2 .visual-preview-stage[data-device=tablet] iframe{width:768px;min-width:768px;max-width:768px}.visual-studio-v2 .visual-preview-stage[data-device=desktop] iframe{width:100%;min-width:1025px;max-width:1440px}',
    'anchos de dispositivos'
  );
  text = replaceExact(
    text,
    '.visual-studio-v2 .visual-preview-stage[data-device=mobile] iframe{width:min(390px,100%);max-width:100%}',
    '.visual-studio-v2 .visual-preview-stage[data-device=mobile] iframe{width:390px;min-width:390px;max-width:390px}',
    'override mobile angosto'
  );
  write(rel, text);
}

// 2) Bump de caché: CSS/JS se sirven immutable por un año.
{
  const rel = 'js/admin/appearance/editor-visual-admin.js';
  let text = read(rel);
  text = replaceExact(
    text,
    `css/admin/visual-studio-v2.css?v=${OLD_VERSION}`,
    `css/admin/visual-studio-v2.css?v=${NEW_VERSION}`,
    'versión CSS desde editor visual'
  );
  write(rel, text);
}
{
  ...
  text = replaceExact(
    text,
    `js/admin/appearance/editor-visual-admin.js?v=${OLD_VERSION}`,
    `js/admin/appearance/editor-visual-admin.js?v=${NEW_VERSION}`,
    'versión JS en admin'
  );
  write(rel, text);
}

// 3) La prueba ya no debe exigir que el iframe se reduzca al ancho del panel.
// Debe comprobar el viewport real dentro del iframe y sus media queries.
{
...
// 4) Hacer que el test forme parte del workflow normal del editor visual.
{
  ...
  text = replaceExact(
    ...
    '"test:visual-builder-ui": "playwright test tests/visual-builder/visual-builder-ui.spec.js tests/visual-builder/visual-builder-runtime.spec.js tests/visual-builder/apariencia-preview-abajo.spec.js --project=chromium"',
    'script test:visual-builder-ui'
  );
  write(rel, text);
}

console.log('Corrección de viewports reales aplicada.');

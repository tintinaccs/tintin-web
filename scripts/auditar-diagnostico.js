...
const builder = read('scripts/construir-manifiesto-diagnostico.js');
...
childProcess.execFileSync(process.execPath, ['scripts/construir-manifiesto-diagnostico.js'], {
  cwd: root,
  stdio: 'pipe'
});
...
check(
  ...
  manifest.pages.length > 0 &&
    ...
    manifest.files.some(file => file.path === '.github/workflows/desplegar-pages.yml')
);
check(

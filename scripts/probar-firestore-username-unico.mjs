...
//
// Mismo control que probar-firestore-telefono-unico.mjs, pero para el
// username: la unicidad no puede depender del código del navegador.
...
try {
  ...
  await ok('Ana reserva su username',
  ...
  await ok('Ana guarda en su perfil el username que reservó',
    updateDoc(doc(ana, 'users', 'ana'), { username: USERNAME_ANA, updatedAt: serverTimestamp() }));
  ...
  await ok('Vaciar el username siempre se puede',
    updateDoc(doc(bea, 'users', 'bea'), { username: '', updatedAt: serverTimestamp() }));
  ...
  await ok('Ana libera su propio username al cambiarlo',
    deleteDoc(doc(ana, 'usernameReservations', USERNAME_ANA)));

  await ok('Liberado, Bea ya lo puede tomar',
    setDoc(doc(bea, 'usernameReservations', USERNAME_ANA), { uid: 'bea', createdAt: serverTimestamp() }));

  console.log(`\nUn username una cuenta: ${checks} controles verificados contra el emulador.`);
} finally {

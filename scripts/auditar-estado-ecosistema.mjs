...
check(
  ...
  pkg.scripts?.['audit:system-health'] === 'node scripts/auditar-estado-ecosistema.mjs && node --test tests/system-health/*.test.mjs' &&
    pkg.scripts?.['audit:final']?.includes('audit:system-health')
);

if (failures) {
  console.error(`\nAuditoría de estado del ecosistema: ${failures} fallo(s).`);
  process.exit(1);
}
console.log('\nAuditoría de estado del ecosistema: autoridad operativa integrada y protegida.');

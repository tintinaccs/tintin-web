...
content = replaceOnce(
  ...
  `    onAuthStateChanged(auth, user => {\n      pageUnsubscribe?.();\n      pageUnsubscribe = null;\n      unregisterUnsavedScope();`,
  'limpieza del scope al cambiar sesión'
);

must(content.includes('`content:${currentPageId}:${currentSectionId}`'), 'Falta el ID dinámico del contenido.');
must(content.includes('window.AdminUnsaved.register(nextId'), 'Falta el registro global del contenido.');
must(content.includes('window.AdminUnsaved?.markDirty(activeUnsavedScopeId)'), 'Falta markDirty global.');
must(!content.includes("window.addEventListener('beforeunload'"), 'Quedó beforeunload local redundante.');
...
integral = replaceOnce(
  ...
  'registro integral del contenido seguro'
);

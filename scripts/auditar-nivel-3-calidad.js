...
const performanceAudit = read('scripts/auditar-rendimiento-tiempo-real.js');
...
check(
  ...
  packageJson.scripts?.['audit:global-responsive-geometry'] === 'node scripts/ejecutar-auditoria-responsive-con-reintento.mjs'
);
check(
  'Existe comando para los siete viewports canónicos',
  packageJson.scripts?.['audit:canonical-viewports'] === 'node scripts/auditar-canonicos-viewports.mjs'
);
...
check(
  ...
  regressionAudit.includes('Tripwire de regresiones') && regressionAudit.includes('auditar-rendimiento-tiempo-real.js')
);
check(

...
check('El loader conserva salida de emergencia', /STORE_GATE_TIMEOUT_MS\s*=\s*\d{3,}/.test(read('js/cargador-pagina.js')) && /RELEASE_TIMEOUT_MS\s*=\s*\d{3,}/.test(read('js/components/color/color-scheme-instant.js')), 'El loader puede quedar infinito.');
...
check('Checkout conserva Resend', read('js/pages/checkout/checkout-email-bridge.js').includes("from '../../email/resend-order-notify.js") && !read('checkout.html').includes('email-notify.js') && !read('checkout.html').includes('resend-order-notify.js'), 'Checkout volvió al canal antiguo.');
check('Checkout protege reintentos después de agotar cuota', read('js/pages/checkout/checkout-quota-guard.js').includes('resource-exhausted') && read('js/pages/checkout/checkout-quota-guard.js').includes('COOLDOWN_MS') && read('js/cargador-mantenimiento-pagina.js').includes('checkout-quota-guard.js'), 'Un 429 permite clics repetidos.');

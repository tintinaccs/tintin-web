...
check('La Fase 10 se inicia desde el runtime compartido', /accesibilidad-global\.js/.test(loader) && /TintinPhase10ImportStarted/.test(loader), 'Debe cargarse una sola vez en todas las páginas que usan page-loader.');
  check('Todas las páginas usan la versión de caché vigente', htmlFiles.every(file => !read(file).includes('js/cargador-pagina.js') || read(file).includes('js/cargador-pagina.js?v=tintin-20260910-global-shell-7')), 'No debe quedar una página con el loader anterior.');
